      ...
      <div className="rounded-2xl border border-white/10 bg-[#0e0f14] p-5">
        ...
        <div className="grid gap-2 sm:grid-cols-2">
          {[
            ...
            { icon: Eye, label: "View profile", action: () => { const keys = ["aztrx-display-name","aztrx-email","aztrx-onboarding-done"]; const data: any = { localStorage: {} }; for (const k of keys) data.localStorage[k] = localStorage.getItem(k); setResult({ ok: true, data, time: 0 }); } },
            { icon: Trash2, label: "Reset onboarding", action: () => { ["aztrx-onboarding-done","aztrx-email-verified","aztrx-display-name"].forEach(k => localStorage.removeItem(k)); setResult({ ok: true, data: { message: "Reset. Refresh dashboard." }, time: 0 }); } },
